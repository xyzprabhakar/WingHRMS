...
{
    /// <summary>
    /// read the data from essl machine essl
    /// </summary>
    public class clsZkemkeepr_read {
        ...
        public List<MachineInfoNew> connectDevice(DateTime FromDate, DateTime ToDate, string MachineIp, int PortNo)
        {
            List<MachineInfoNew> lstMachineInfo = null;
            //Check is IP is valid
            if (!UniversalStatic.ValidateIP(MachineIp))
            {
                return lstMachineInfo;
                //throw new Exception("machine IP is not valid");
            }
            //check machine is ping or not
            if (!UniversalStatic.PingTheDevice(MachineIp))
            {
                return lstMachineInfo;
                //throw new Exception("machine "+ MachineIp + " is not connected");
            }
            int MachineID = 0;
            objZkeeper = new ZkemClient(RaiseDeviceEvent);
            IsDeviceConnected = objZkeeper.Connect_Net(MachineIp, PortNo);
            if (IsDeviceConnected)
            {
                
                lstMachineInfo = manipulator.GetLogData(objZkeeper, MachineID, MachineIp, FromDate,ToDate).ToList();
    ...
    internal class DeviceManipulator
    {
        ...
        public ICollection<MachineInfoNew> GetLogData(ZkemClient objZkeeper, int machineNumber, string machineIp, DateTime fromtime , DateTime totime)
        {
            ...
            ICollection<MachineInfoNew> lstEnrollData = new List<MachineInfoNew>();
            ...
            {
                DateTime inputDate_ = new DateTime(dwYear, dwMonth, dwDay, dwHour, dwMinute, dwSecond);
                if (0 >= DateTime.Compare(fromtime, inputDate_) && 0 >= DateTime.Compare(inputDate_, totime))
                {
                    string inputDate = inputDate_.ToString();

                    MachineInfoNew objInfo = new MachineInfoNew();
                    objInfo.MachineNumber = machineNumber;
                    //objInfo.IndRegID = int.Parse(dwEnrollNumber1).ToString("D6");
                    objInfo.IndRegID = dwEnrollNumber1;
                    objInfo.DateTimeRecord = inputDate_;
                    objInfo.MachineIP = machineIp;
